...
### 204 million LESS people ID as white
totals_2010 %>% 
...
rural_states <- rural_counties %>% 
  ...
  rural_counties %>% 
  mutate(STATE = stringr::str_extract(NAME, "\\b[A-Za-z]+$")) %>% 
  group_by(STATE) %>% 
  reframe(count = n()) %>% 
  arrange(desc(count)) %>% 
  head(n=15) %>% 
...
rural_states_geo <- rural_states %>%
  mutate(state = trimws(state),  # Removes spaces
         region = tolower(state)) %>% 
  left_join(states_map, by="region")

### plot rural states/why not counties ??
ggplot(rural_states_geo, aes(x = long, y = lat, group = group, fill = counties)) +
...
units_wide <- units_dec %>% 
  select(-GEOID) %>% 
  tidyr::pivot_wider(names_from = variable, values_from = units) 
...
### top growing cities
top_cities <- msa_flow %>% 
  mutate(FULL1_NAME = stringr::str_remove(FULL1_NAME, " Metro Area")) %>%  
...
mortgage_geo_clean <- mortgage_geo %>% 
  left_join(acs_2023, by=c("variable" = "name")) %>% 
...
# Rename the 'variable' column to 'name' in the metadata for joining
acs_vars <- acs_vars %>%
  rename(variable = name)

### total US employment data by occupation numbers
employment_data <- get_acs(
  geography = "us",
  table = "B24114",  # Occupation data by number of ppl
  year = 2022,
  survey = "acs5",
  output = "tidy"
)

# Join the labels to your employment data
# Join the labels to your employment data
employment_data_labeled <- employment_data %>%
  left_join(acs_vars, by = "variable") %>%
  select(GEOID, NAME, label, estimate, moe) # Keep only relevant columns

# Filter and clean the data for meaningful analysis
employment_data_clean <- employment_data_labeled %>%
  mutate(
    label_clean = str_extract(label, "[^!!]+$")  # Extract everything after the last '!!'
  ) %>%
  select(GEOID, NAME, label_clean, estimate, moe) 

employment_data_clean %>% 
  filter(!label_clean == "Total:") %>% 
  mutate(label_clean = str_trunc(label_clean, width = 40, side = "right")) %>%  # Truncate to 15 characters
  arrange(desc(estimate)) %>% 
  head(n=15) %>% 
  ggplot(aes(forcats::fct_reorder(label_clean, estimate), estimate/1000, fill=label_clean)) +
  geom_col() +
  guides(fill="none") +
  labs(title = "Most Common Jobs", y="Millions of People", x="",
       caption = "Source: Census, tidycensus") +
...
### jobs by state
### total US employment data by occupation numbers
employment_state <- get_acs(
  geography = "state",
  table = "C24020",  # Occupation data by number of ppl
  year = 2022,
  survey = "acs5",
  output = "tidy"
)
...
# Join the labels to your employment data
# Join the labels to your employment data
employment_state_labeled <- employment_state %>%
  left_join(acs_vars, by = "variable") %>%
  select(GEOID, NAME, label, estimate, moe)

# Optional: Visualize the data geo-map
states <- map_data("state")

### clean df for lower name
state_jobs <- employment_state_labeled %>%
  mutate(region = tolower(str_trim(NAME))) %>% 
  select(-NAME) 

### just get the most common job per state
state_jobs_agg <- state_jobs %>%
  # Extract everything after the last ':' in the label
  mutate(
    label_clean = str_extract(label, "[^:]+$")) %>%
  # Remove non-occupational labels
  filter(!is.na(label_clean),
    !label_clean %in% c("Total", "Male", "Female")) %>%
  # Group by region and occupation
  group_by(region, label_clean) %>%
  # Summarize total estimates and margin of error
  reframe(
    total_estimate = sum(estimate, na.rm = TRUE),
    total_moe = sqrt(sum(moe^2, na.rm = TRUE)),
    .groups = "drop") %>%
  # Sort within each region
  arrange(region, desc(total_estimate))

# Get the most common job per state
top_jobs_state <- state_jobs_agg %>%
  group_by(region) %>%
  slice_max(total_estimate, n = 1)

### merge state map w df
state_jobs_geo <- top_jobs_state %>% 
  left_join(states, by="region")

ggplot(state_jobs_geo, aes(long, lat, group = group, fill = label_clean)) +
  geom_polygon(color = "white", size = 0.2) +  # Map polygons
  coord_fixed(1.3) +  # Fix aspect ratio
  scale_fill_brewer(palette = "Set3", name = "Occupation") +  # Add color scale
  labs(title = "Most Common Occupation by State",
       x = NULL, y = NULL) +
  ggthemes::theme_map() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
